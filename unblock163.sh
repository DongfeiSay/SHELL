...
#	版本: 1.0.6
#	作者: DongfeiSay
#	地址: https://git.moesay.cf/DongfeiSay/SHELL
# --------------------------------------------------------------
...
_DOWNLOAD(){
	...
	env GIT_SSL_NO_VERIFY=true git clone -b master https://git.moesay.cf/nondanee/UnblockNeteaseMusic.git "${FOLDER}"
...
_SERVICE(){
	if [[ "${SYSTEM_RELEASE}" = "centos" ]]; then
		wget --no-check-certificate "https://raw.moesay.cf/DongfeiSay/SHELL/master/service/${NAME_SERVICE}_centos" -O "/etc/init.d/${NAME_SERVICE}"
		...
		wget --no-check-certificate "https://raw.moesay.cf/DongfeiSay/SHELL/master/service/${NAME_SERVICE}_debian" -O "/etc/init.d/${NAME_SERVICE}"
		[[ !  -e "/etc/init.d/${NAME_SERVICE}" ]] && echo -e "${ERROR} ${NAME} 服务管理脚本下载失败 !" && _INSTALLATION_FAILURE_CLEANUP
		chmod +x "/etc/init.d/${NAME_SERVICE}"
...
# 更新脚本
_UPDATE_SHELL(){
	NEW_VER_SHELL=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.moesay.cf/DongfeiSay/SHELL/master/unblock163.sh"|grep 'NOW_VER_SHELL="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z "${NEW_VER_SHELL}" ]] && echo -e "${ERROR} 获取脚本最新版本失败！无法链接到 Github !" && exit 1
		...
		wget -N --no-check-certificate "https://raw.moesay.cf/DongfeiSay/SHELL/master/unblock163.sh"
